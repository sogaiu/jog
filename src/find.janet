      ...
      (when-let [head (first parsed)]
        (when (symbol? head)
          ...
          (cond
            (and (get special-definers head)
                 (<= 4 (length parsed))) # metadata possible
            (when-let [ds-tbl (find-special-docstring (j/down next-zloc))]
              (array/push results
                          (merge ds-tbl {:def-type (string head)
                                         :name name})))
            #
            (get call-definers head)
            (when-let [ds-tbl (find-caller-docstring (j/down next-zloc))]
              (array/push results
                          (merge ds-tbl {:def-type (string head)
                                         :name name})))
            #
            (and (= 'defdyn head) (= 3 (length parsed)))
            (when-let [ds-tbl (find-defdyn-docstring (j/down next-zloc))]
              (array/push results
                          (merge ds-tbl {:def-type (string head)
                                         :name name})))
            # XXX: other cases?
